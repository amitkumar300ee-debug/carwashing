// ───────────── HydroShine Shared App Logic ─────────────
const WA = "919469793349";

// ⚠️ SET THIS after deploying your Cloudflare Worker:
const BACKEND_URL = ""; // e.g. " https://hydroshine-api.amitkumar300ee.workers.dev"
...
// ───────────── BOOKING SUBMIT (used on booking.html) ─────────────
async function bookNow() {
  ...
  // Determine which booking method the user picked
...
function openWA(d) {
  const msg = `🚗 *New Booking – HydroShine Jammu*\n\n👤 ${d.name}\n📞 ${d.phone}\n🛠️ ${d.service}\n➕ ${d.addon}\n📅 ${d.date} · ${d.time}\n📍 ${d.address}\n📝 ${d.notes}`;
...
// ───────────── PRE-FILL BOOKING FORM (from query params, e.g. service=Bike) ─────────────
...
// Looks for elements with [data-price="path.to.value"] and fills textContent.
// Example: <span data-price="services.hatchback.price_min">299</span>
// Falls back silently to existing static content if backend unavailable.
...
function injectDesktopNav(activePage) {
  ...
  // Insert nav between logo and right-side button
  ...
  // Account icon link on desktop (since bottom nav is hidden)
